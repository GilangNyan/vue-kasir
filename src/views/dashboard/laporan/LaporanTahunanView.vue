...
export default {
    ...
    methods: {
        ...
        generateTahun() {
            ...
            for (let tahun = tahunIni; tahun >= 2000; tahun--) {
                this.tahun.push(tahun)
            }
        },
        setCurrentYear() {
...
<template>
    <div class="space-y-4">
        ...
        <div class="w-full p-2 bg-neutral-100 dark:bg-slate-700 border rounded-lg dark:border-slate-600 flex justify-between">
            <div class="space-x-2">
                <select name="tahun" id="tahun" class="border p-2 w-48 text-sm rounded-lg shadow-sm dark:bg-slate-800 dark:border-slate-600 dark:text-slate-200 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 invalid:text-red-500 focus:invalid:border-red-500 focus:invalid:ring-red-500 placeholder:text-neutral-400 dark:placeholder:text-slate-500" v-model="selectedTahun">
                    <option :value="item" v-for="item in tahun" :key="item">{{ item }}</option>
                </select>
                ...
                <button type="button" class="bg-white hover:bg-neutral-200 dark:bg-slate-700 dark:hover:bg-slate-800 border rounded-lg dark:border-slate-600 dark:text-white p-2 text-sm" @click="setCurrentYear()">Tahun Ini</button>
            </div>
        ...
        <h2 class="text-xl font-bold dark:text-slate-200">Laporan Tahunan {{ selectedTahun }}</h2>
            ...
            <table class="table-auto w-full relative">
                <thead class="border-b-2 bg-neutral-100 dark:bg-slate-700 dark:border-slate-600">
                    <tr>
                        ...
                        <th class="p-3 text-sm font-semibold tracking-wide text-center dark:text-slate-200">Bulan</th>
                        <th class="p-3 text-sm font-semibold tracking-wide text-right dark:text-slate-200">Jumlah Faktur</th>
                        ...
                        <th class="p-3 text-sm font-semibold tracking-wide text-left dark:text-slate-200 w-24">Opsi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-neutral-100 dark:divide-slate-600">
                    <tr v-if="transaksi.length != 0" v-for="(item, index) in transaksi">
                        ...
                        <td class="p-3 text-sm text-neutral-700 dark:text-slate-300 text-center">{{ item.bulan }}</td>
                        ...
                        <td class="p-3 text-sm text-neutral-700 dark:text-slate-300 text-right">Rp. {{ item.subtotal - item.diskonRp }}</td>
                    ...
                    <tr v-else>
                        <td colspan="9" class="p-3 text-sm text-neutral-700 dark:text-slate-300 text-center">Belum ada data transaksi untuk tahun ini.</td>
                    </tr>
                </tbody>
                <tfoot class="border-t-2 bg-neutral-100 dark:bg-slate-700 dark:border-slate-600">
                    <tr v-if="transaksi.length != 0">
                        ...
                        <td class="p-3 text-sm text-neutral-700 dark:text-slate-300 text-right font-semibold">Rp. {{ totalledValue.total - totalledValue.diskonRp }}</td>
